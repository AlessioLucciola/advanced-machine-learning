...
# learning rate, number of training epochs, and regularization strength. You
...
for lr, hs, ni, rg in product(learning_rates, hidden_sizes, num_iterations, regs):
    print('Current configuration: learning rate=' + str(lr) +', hidden layer size=' + str(hs) + ', num.iterations=' + str(ni) + ', reg.strenght=' + str(rg))
    net = TwoLayerNet(input_size, hs, num_classes)
    # Train the network
    stats = net.train(X_train, y_train, X_val, y_val,
                num_iters=ni, batch_size=200,
    ...
    if val_acc > best_accuracy:
        ...
        best_hyperparameters['num_iterations'] = ni
        best_hyperparameters['regs'] = rg
        best_net = net
